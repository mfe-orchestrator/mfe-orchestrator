# Graph Report - mfe-orchestrator  (2026-08-20)

## Corpus Check
- 436 files · ~402,362 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3700 nodes · 7356 edges · 213 communities (167 shown, 46 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 44 edges (avg confidence: 0.73)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b925e423`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- [[_COMMUNITY_Deployment Service & Models|Deployment Service & Models]]
- [[_COMMUNITY_Frontend Dependencies|Frontend Dependencies]]
- [[_COMMUNITY_Backend Controllers & Auth|Backend Controllers & Auth]]
- [[_COMMUNITY_Backend Error Types|Backend Error Types]]
- [[_COMMUNITY_Cloud Storage & Project Controllers|Cloud Storage & Project Controllers]]
- [[_COMMUNITY_UI Cards & Alerts|UI Cards & Alerts]]
- [[_COMMUNITY_Biome Configuration|Biome Configuration]]
- [[_COMMUNITY_Azure Storage & Microfrontend Model|Azure Storage & Microfrontend Model]]
- [[_COMMUNITY_Auth Pages & Form Inputs|Auth Pages & Form Inputs]]
- [[_COMMUNITY_Code Repositories UI|Code Repositories UI]]
- [[_COMMUNITY_Root Package Config|Root Package Config]]
- [[_COMMUNITY_Project & Environment APIs|Project & Environment APIs]]
- [[_COMMUNITY_Microfrontend Pages & API|Microfrontend Pages & API]]
- [[_COMMUNITY_Backend Dependencies|Backend Dependencies]]
- [[_COMMUNITY_Project Users & Invitations|Project Users & Invitations]]
- [[_COMMUNITY_Biome Suspicious Rules|Biome Suspicious Rules]]
- [[_COMMUNITY_Dialogs & Color Picker|Dialogs & Color Picker]]
- [[_COMMUNITY_UI Inputs & Popover|UI Inputs & Popover]]
- [[_COMMUNITY_Frontend Routes|Frontend Routes]]
- [[_COMMUNITY_Backend Controller Layer|Backend Controller Layer]]
- [[_COMMUNITY_Tables & Layout Components|Tables & Layout Components]]
- [[_COMMUNITY_Sidebar & Theme Toggle|Sidebar & Theme Toggle]]
- [[_COMMUNITY_ApiKeyService.ts|ApiKeyService.ts]]
- [[_COMMUNITY_Frontend API Client|Frontend API Client]]
- [[_COMMUNITY_Auth Wrappers & Context|Auth Wrappers & Context]]
- [[_COMMUNITY_Project Docs & Tooling|Project Docs & Tooling]]
- [[_COMMUNITY_Biome Correctness Rules|Biome Correctness Rules]]
- [[_COMMUNITY_Tabs Component|Tabs Component]]
- [[_COMMUNITY_API Key Domain|API Key Domain]]
- [[_COMMUNITY_API Hooks & Pages|API Hooks & Pages]]
- [[_COMMUNITY_Code Repositories API|Code Repositories API]]
- [[_COMMUNITY_Code Repository Service|Code Repository Service]]
- [[_COMMUNITY_Login & Social Buttons|Login & Social Buttons]]
- [[_COMMUNITY_Select & Environment Selector|Select & Environment Selector]]
- [[_COMMUNITY_Frontend App TSConfig|Frontend App TSConfig]]
- [[_COMMUNITY_Dropdown Menu & Sidebar|Dropdown Menu & Sidebar]]
- [[_COMMUNITY_Google & S3 Storage Clients|Google & S3 Storage Clients]]
- [[_COMMUNITY_GitHub Client Methods|GitHub Client Methods]]
- [[_COMMUNITY_Frontend Dev Dependencies|Frontend Dev Dependencies]]
- [[_COMMUNITY_Deployments & Variables API|Deployments & Variables API]]
- [[_COMMUNITY_Azure DevOps Client Types|Azure DevOps Client Types]]
- [[_COMMUNITY_E2E Package Config|E2E Package Config]]
- [[_COMMUNITY_AuthenticationMethod.ts|AuthenticationMethod.ts]]
- [[_COMMUNITY_E2E TSConfig|E2E TSConfig]]
- [[_COMMUNITY_FrontendIntegration.tsx|FrontendIntegration.tsx]]
- [[_COMMUNITY_Canary Users Domain|Canary Users Domain]]
- [[_COMMUNITY_Form Input Fields|Form Input Fields]]
- [[_COMMUNITY_Environment List & Drag-Drop|Environment List & Drag-Drop]]
- [[_COMMUNITY_Frontend Node TSConfig|Frontend Node TSConfig]]
- [[_COMMUNITY_Telemetry Service & Config|Telemetry Service & Config]]
- [[_COMMUNITY_User & Invitation API|User & Invitation API]]
- [[_COMMUNITY_integration|integration]]
- [[_COMMUNITY_Azure DevOps Client|Azure DevOps Client]]
- [[_COMMUNITY_Error Handler Middleware|Error Handler Middleware]]
- [[_COMMUNITY_Fastify App Bootstrap|Fastify App Bootstrap]]
- [[_COMMUNITY_UserModel.ts|UserModel.ts]]
- [[_COMMUNITY_useUserApi.ts|useUserApi.ts]]
- [[_COMMUNITY_Project Wizard Service|Project Wizard Service]]
- [[_COMMUNITY_Backend TSConfig|Backend TSConfig]]
- [[_COMMUNITY_Code Repository Model|Code Repository Model]]
- [[_COMMUNITY_CodeRepositoryController.ts|CodeRepositoryController.ts]]
- [[_COMMUNITY_MicrofrontendController.ts|MicrofrontendController.ts]]
- [[_COMMUNITY_Telemetry Flags & Plugins|Telemetry Flags & Plugins]]
- [[_COMMUNITY_Frontend App Entry & Theme|Frontend App Entry & Theme]]
- [[_COMMUNITY_Frontend Base TSConfig|Frontend Base TSConfig]]
- [[_COMMUNITY_MicrofrontendService.ts|MicrofrontendService.ts]]
- [[_COMMUNITY_IntegrationService.ts|IntegrationService.ts]]
- [[_COMMUNITY_errorHandler.ts|errorHandler.ts]]
- [[_COMMUNITY_Backend Dev Dependencies|Backend Dev Dependencies]]
- [[_COMMUNITY_AuthenticationMethod.ts|AuthenticationMethod.ts]]
- [[_COMMUNITY_Microfrontend Build Dialog|Microfrontend Build Dialog]]
- [[_COMMUNITY_integration|integration]]
- [[_COMMUNITY_Unreleased|[Unreleased]]]
- [[_COMMUNITY_DeploymentCanaryUsersController.ts|DeploymentCanaryUsersController.ts]]
- [[_COMMUNITY_Backend Test TSConfig|Backend Test TSConfig]]
- [[_COMMUNITY_Biome Style Rules|Biome Style Rules]]
- [[_COMMUNITY_AuthenticationMethod.ts|AuthenticationMethod.ts]]
- [[_COMMUNITY_Switch.rhf.tsx|Switch.rhf.tsx]]
- [[_COMMUNITY_EnvironmentVariableForm.tsx|EnvironmentVariableForm.tsx]]
- [[_COMMUNITY_AzureDevOpsClient.ts|AzureDevOpsClient.ts]]
- [[_COMMUNITY_BusinessException|BusinessException]]
- [[_COMMUNITY_FastifyInstance|FastifyInstance]]
- [[_COMMUNITY_App.tsx|App.tsx]]
- [[_COMMUNITY_UserService|UserService]]
- [[_COMMUNITY_Backend Postbuild Script|Backend Postbuild Script]]
- [[_COMMUNITY_Dependencies.tsx|Dependencies.tsx]]
- [[_COMMUNITY_autorization.ts|autorization.ts]]
- [[_COMMUNITY_Sidebar.tsx|Sidebar.tsx]]
- [[_COMMUNITY_IMarket|IMarket]]
- [[_COMMUNITY_Backend Architecture Patterns|Backend Architecture Patterns]]
- [[_COMMUNITY_IntegrationService.ts|IntegrationService.ts]]
- [[_COMMUNITY_Switch Component|Switch Component]]
- [[_COMMUNITY_React Misc Hooks|React Misc Hooks]]
- [[_COMMUNITY_MicrofrontendModel.ts|MicrofrontendModel.ts]]
- [[_COMMUNITY_ApiStatusHandler Component|ApiStatusHandler Component]]
- [[_COMMUNITY_CICD Deploy Workflows|CI/CD Deploy Workflows]]
- [[_COMMUNITY_E2E Test Workflow|E2E Test Workflow]]
- [[_COMMUNITY_reCAPTCHA Utility|reCAPTCHA Utility]]
- [[_COMMUNITY_AWS Provider Branding|AWS Provider Branding]]
- [[_COMMUNITY_Registration E2E Test|Registration E2E Test]]
- [[_COMMUNITY_app|app]]
- [[_COMMUNITY_Example E2E Fixtures|Example E2E Fixtures]]
- [[_COMMUNITY_Azure Provider Branding|Azure Provider Branding]]
- [[_COMMUNITY_Google Cloud Branding|Google Cloud Branding]]
- [[_COMMUNITY_Web App Manifest|Web App Manifest]]
- [[_COMMUNITY_Global Config Types|Global Config Types]]
- [[_COMMUNITY_Azure DevOps Branding|Azure DevOps Branding]]
- [[_COMMUNITY_Project New DTO|Project New DTO]]
- [[_COMMUNITY_Frontend HTML Entry|Frontend HTML Entry]]
- [[_COMMUNITY_GitLab Branding|GitLab Branding]]
- [[_COMMUNITY_Local Installation for development 🛠️|Local Installation for development 🛠️]]
- [[_COMMUNITY_Wizard Basic Props|Wizard Basic Props]]
- [[_COMMUNITY_API Response DTO|API Response DTO]]
- [[_COMMUNITY_Vite Env Types|Vite Env Types]]
- [[_COMMUNITY_Vite Config|Vite Config]]
- [[_COMMUNITY_Start Script|Start Script]]
- [[_COMMUNITY_Start Complete Script|Start Complete Script]]
- [[_COMMUNITY_Multipart Plugin|Multipart Plugin]]
- [[_COMMUNITY_GitlabClient.ts|GitlabClient.ts]]
- [[_COMMUNITY_canary_users|canary_users]]
- [[_COMMUNITY_mfe-orchestrator Helm chart|mfe-orchestrator Helm chart]]
- [[_COMMUNITY_Security Policy|Security Policy]]
- [[_COMMUNITY_common|common]]
- [[_COMMUNITY_README|README.md]]
- [[_COMMUNITY_start-backend.sh|start-backend.sh]]
- [[_COMMUNITY_deployments|deployments]]
- [[_COMMUNITY_projectWizardStateMachine.ts|projectWizardStateMachine.ts]]
- [[_COMMUNITY_GitHub Actions Workflows|GitHub Actions Workflows]]
- [[_COMMUNITY_scripts|scripts]]
- [[_COMMUNITY_FederationConfigService|FederationConfigService]]
- [[_COMMUNITY_2.3.0 - 2026-08-11|[2.3.0] - 2026-08-11]]
- [[_COMMUNITY_Monorepo Architecture (pnpm workspace + Turbo)|Monorepo Architecture (pnpm workspace + Turbo)]]
- [[_COMMUNITY_package.json|package.json]]
- [[_COMMUNITY_entrypoint.sh|entrypoint.sh]]
- [[_COMMUNITY_CLAUDE|CLAUDE.md]]
- [[_COMMUNITY_Conventional Commits Specification|Conventional Commits Specification]]
- [[_COMMUNITY_start-mongod.sh|start-mongod.sh]]
- [[_COMMUNITY_start-redis.sh|start-redis.sh]]
- [[_COMMUNITY_ApiStatusHandler Data Fetching Pattern|ApiStatusHandler Data Fetching Pattern]]
- [[_COMMUNITY_Backend Layered Architecture (Models - Services - Controllers - Plugins)|Backend Layered Architecture (Models -> Services -> Controllers -> Plugins)]]
- [[_COMMUNITY_BaseAuthorizedService Authorization Pattern|BaseAuthorizedService Authorization Pattern]]
- [[_COMMUNITY_Multi-provider Authentication (Local JWT, Auth0, Google OAuth, Azure EntraID, API keys)|Multi-provider Authentication (Local JWT, Auth0, Google OAuth, Azure EntraID, API keys)]]
- [[_COMMUNITY_GitHub Actions Workflows Guide|GitHub Actions Workflows Guide]]
- [[_COMMUNITY_Windsurf Project Rules|Windsurf Project Rules]]
- [[_COMMUNITY_TelemetryService.collect (payload builder)|TelemetryService.collect (payload builder)]]
- [[_COMMUNITY_TelemetryDTO (payload type)|TelemetryDTO (payload type)]]
- [[_COMMUNITY_Semantic Versioning (SemVer)|Semantic Versioning (SemVer)]]
- [[_COMMUNITY_Telemetry Installation ID|Telemetry Installation ID]]
- [[_COMMUNITY_Telemetry Payload (aggregate counters only)|Telemetry Payload (aggregate counters only)]]
- [[_COMMUNITY_Microfrontend Orchestrator (product)|Microfrontend Orchestrator (product)]]
- [[_COMMUNITY_scripts|scripts]]
- [[_COMMUNITY_environmentVariables|environmentVariables]]
- [[_COMMUNITY_platform.json|platform.json]]
- [[_COMMUNITY_dashboard|dashboard]]
- [[_COMMUNITY_CreateApiKeyForm.tsx|CreateApiKeyForm.tsx]]
- [[_COMMUNITY_project|project]]
- [[_COMMUNITY_dashboard|dashboard]]
- [[_COMMUNITY_theme|theme]]
- [[_COMMUNITY_form|form]]
- [[_COMMUNITY_environmentVariables|environmentVariables]]
- [[_COMMUNITY_project|project]]
- [[_COMMUNITY_platform.json|platform.json]]
- [[_COMMUNITY_Local Installation for development 🛠️|Local Installation for development 🛠️]]
- [[_COMMUNITY_EmailSenderService|EmailSenderService]]
- [[_COMMUNITY_environment|environment]]
- [[_COMMUNITY_settings|settings]]
- [[_COMMUNITY_theme|theme]]
- [[_COMMUNITY_build|build]]
- [[_COMMUNITY_delete|delete]]
- [[_COMMUNITY_errorHandler.ts|errorHandler.ts]]
- [[_COMMUNITY_project_invitation|project_invitation]]
- [[_COMMUNITY_project_invitation|project_invitation]]
- [[_COMMUNITY_AzureStorageClient|AzureStorageClient]]
- [[_COMMUNITY_NpmRegistryClient.ts|NpmRegistryClient.ts]]
- [[_COMMUNITY_stats|stats]]
- [[_COMMUNITY_marketing|marketing]]
- [[_COMMUNITY_card|card]]
- [[_COMMUNITY_BaseAuthorizedService|BaseAuthorizedService]]
- [[_COMMUNITY_DeploymentService|DeploymentService]]
- [[_COMMUNITY_GitlabClient.ts|GitlabClient.ts]]
- [[_COMMUNITY_table|table]]
- [[_COMMUNITY_form|form]]
- [[_COMMUNITY_FederationConfigService|FederationConfigService]]
- [[_COMMUNITY_curl|curl]]
- [[_COMMUNITY_NpmRegistryClient.ts|NpmRegistryClient.ts]]
- [[_COMMUNITY_.getMicrofrontendVersion|.getMicrofrontendVersion]]
- [[_COMMUNITY_.create|.create]]
- [[_COMMUNITY_EmailSenderService|EmailSenderService]]
- [[_COMMUNITY_validation|validation]]
- [[_COMMUNITY_GoogleStorageClient|GoogleStorageClient]]
- [[_COMMUNITY_S3BucketClient|S3BucketClient]]
- [[_COMMUNITY_personalData|personalData]]
- [[_COMMUNITY_Examples|Examples]]
- [[_COMMUNITY_1.0.0 - 2025-12-07|[1.0.0] - 2025-12-07]]
- [[_COMMUNITY_curl|curl]]
- [[_COMMUNITY_assist|assist]]
- [[_COMMUNITY_2.1.0 - 2026-08-06|[2.1.0] - 2026-08-06]]
- [[_COMMUNITY_3.0.0 - 2026-08-16|[3.0.0] - 2026-08-16]]
- [[_COMMUNITY_Unreleased|[Unreleased]]]
- [[_COMMUNITY_Scopes (Optional)|Scopes (Optional)]]
- [[_COMMUNITY_DeploymentController.ts|DeploymentController.ts]]

## God Nodes (most connected - your core abstractions)
1. `toObjectId()` - 126 edges
2. `useToastNotificationStore` - 74 edges
3. `Button()` - 67 edges
4. `BaseAuthorizedService` - 57 edges
5. `useProjectStore` - 53 edges
6. `createBusinessException()` - 50 edges
7. `IMicrofrontend` - 46 edges
8. `CodeRepositoryService` - 45 edges
9. `apiKeys` - 45 edges
10. `apiKeys` - 45 edges

## Surprising Connections (you probably didn't know these)
- `MongoDB Docker Service (local development, port 27018)` --semantically_similar_to--> `MongoDB Docker Service (production compose)`  [INFERRED] [semantically similar]
  docker-local/docker-compose-development.yaml → docker-compose.yaml
- `Redis Docker Service (local development, port 6380)` --semantically_similar_to--> `Redis Docker Service (production compose)`  [INFERRED] [semantically similar]
  docker-local/docker-compose-development.yaml → docker-compose.yaml
- `Dependabot Weekly npm Updates (frontend and backend)` --conceptually_related_to--> `Security Policy`  [INFERRED]
  .github/dependabot.yml → SECURITY.md
- `encryptedFields()` --indirect_call--> `document()`  [INFERRED]
  backend/src/utils/encryptedFieldsPlugin.ts → backend/src/utils/globalVariablesScript.test.ts
- `Lefthook Pre-commit Hooks (Biome check + typecheck)` --conceptually_related_to--> `Monorepo Architecture (pnpm workspace + Turbo)`  [INFERRED]
  lefthook.yml → .github/copilot-instructions.md

## Import Cycles
- 3-file cycle: `frontend/src/pages/integration/partials/index.ts -> frontend/src/pages/integration/partials/views/index.ts -> frontend/src/pages/integration/partials/views/FrontendIntegration.tsx -> frontend/src/pages/integration/partials/index.ts`
- 3-file cycle: `frontend/src/authentication/components/LoginPage.tsx -> frontend/src/authentication/components/SocialLoginRow.tsx -> frontend/src/authentication/components/LoginWithMicrosoftButton.tsx -> frontend/src/authentication/components/LoginPage.tsx`
- 3-file cycle: `frontend/src/authentication/components/LoginPage.tsx -> frontend/src/authentication/components/SocialLoginRow.tsx -> frontend/src/authentication/components/LoginWithGoogleButton.tsx -> frontend/src/authentication/components/LoginPage.tsx`
- 3-file cycle: `frontend/src/authentication/components/LoginPage.tsx -> frontend/src/authentication/components/SocialLoginRow.tsx -> frontend/src/authentication/components/LoginWithAuth0Button.tsx -> frontend/src/authentication/components/LoginPage.tsx`
- 3-file cycle: `frontend/src/hooks/apiClients/useProjectApi.ts -> frontend/src/hooks/useApiClient.tsx -> frontend/src/store/useProjectStore.ts -> frontend/src/hooks/apiClients/useProjectApi.ts`
- 3-file cycle: `frontend/src/hooks/apiClients/useEnvironmentsApi.ts -> frontend/src/hooks/useApiClient.tsx -> frontend/src/store/useProjectStore.ts -> frontend/src/hooks/apiClients/useEnvironmentsApi.ts`
- 4-file cycle: `frontend/src/hooks/apiClients/useEnvironmentsApi.ts -> frontend/src/hooks/useApiClient.tsx -> frontend/src/store/useProjectStore.ts -> frontend/src/hooks/apiClients/useProjectApi.ts -> frontend/src/hooks/apiClients/useEnvironmentsApi.ts`

## Hyperedges (group relationships)
- **CI/CD Pipeline (build, dockerize, deploy, e2e)** — _github_workflows_monorepo_build_build_and_test, _github_workflows_docker_build_docker_build, _github_workflows_deploy_deploy_to_server, _github_workflows_e2e_tests_e2e_tests [EXTRACTED 1.00]
- **Code Quality and Commit Convention Enforcement** — lefthook_pre_commit_hooks, lefthook_commit_msg_commitlint, commit_conventions_conventional_commits, commit_conventions_semantic_versioning, changelog_changelog [INFERRED 0.85]

## Communities (213 total, 46 thin omitted)

### Community 0 - "Deployment Service & Models"
Cohesion: 0.11
Nodes (4): IEnvironment, escapeRegExp(), getMicrofrontendUrlCanary(), ServeService

### Community 1 - "Frontend Dependencies"
Cohesion: 0.06
Nodes (32): dependencies, @auth0/auth0-react, axios, @azure/msal-browser, @azure/msal-react, clsx, date-fns, @heroicons/react (+24 more)

### Community 2 - "Backend Controllers & Auth"
Cohesion: 0.11
Nodes (29): Auth0AuthWrapper(), Auth0AuthWrapperProps, FormValues, LoginComponentProps, LoginPage(), LoginWithAuth0Button(), LoginWithGoogleButton(), LoginWithMicrosoftButton() (+21 more)

### Community 3 - "Backend Error Types"
Cohesion: 0.10
Nodes (19): ColorPickerProps, EnvironmentSelectorProps, EnvironmentDTO, moveItem(), useDragAndDropOrder(), UseDragAndDropOrderProps, EnvironmentList(), EnvironmentListProps (+11 more)

### Community 4 - "Cloud Storage & Project Controllers"
Cohesion: 0.15
Nodes (13): CreateBuildRequest, codeRepositoryController(), azureDataSchema, CODE_REPOSITORY_SECRET_PATHS, codeRepositorySchema, CodeRepositoryType, githubDataSchema, gitlabDataSchema (+5 more)

### Community 6 - "Biome Configuration"
Cohesion: 0.16
Nodes (13): ALLOWED_AVATAR_MIME_TYPES, IUserAvatar, IUserAvatarDocument, userAvatarSchema, RegisterUserData, MicrofrontendUploadDTO, ResetPasswordDataDTO, ResetPasswordRequestDTO (+5 more)

### Community 7 - "Azure Storage & Microfrontend Model"
Cohesion: 0.21
Nodes (6): IWizardProjectState, WizardProjectState, WizardProjectStateSchema, ProjectCreateInput, ProjectWizardService, NewProjectWizardContext

### Community 8 - "Auth Pages & Form Inputs"
Cohesion: 0.11
Nodes (18): DeleteConfirmationDialog(), DeleteConfirmationDialogProps, ApiKey, CreateApiKeyDTO, CreateApiKeyResponseDTO, useApiKeysApi(), CanaryUser, useCanaryUsersApi() (+10 more)

### Community 9 - "Code Repositories UI"
Cohesion: 0.06
Nodes (37): AuthenticationLayout(), AuthenticationLayoutProps, sizeClasses, CreateOrganizationForm(), CreateOrganizationFormValues, TextFieldProps, MainLogo(), MainLogoProps (+29 more)

### Community 10 - "Root Package Config"
Cohesion: 0.05
Nodes (37): devDependencies, @biomejs/biome, @commitlint/cli, @commitlint/config-conventional, lefthook, turbo, license, packageManager (+29 more)

### Community 11 - "Project & Environment APIs"
Cohesion: 0.25
Nodes (8): customSource, customUrl, mfeOrchestratorHub, microfrontend, host, hostTypes, no_microfrontends_found, no_microfrontends_found_description

### Community 12 - "Microfrontend Pages & API"
Cohesion: 0.11
Nodes (20): formatter, arrowParentheses, attributePosition, bracketSameLine, bracketSpacing, enabled, expand, formatWithErrors (+12 more)

### Community 13 - "Backend Dependencies"
Cohesion: 0.06
Nodes (34): dependencies, @aws-sdk/client-s3, @aws-sdk/s3-request-presigner, axios, @azure/identity, @azure/storage-blob, bcryptjs, dotenv (+26 more)

### Community 14 - "Project Users & Invitations"
Cohesion: 0.10
Nodes (29): CanaryDeploymentType, CanaryType, ICanaryMicrofrontend, ICodeRepositoryMicrofrontend, IHostMicrofrontend, IMicrofrontendStack, IPosition, microfrontendCanaryTypeSchema (+21 more)

### Community 15 - "Biome Suspicious Rules"
Cohesion: 0.06
Nodes (34): suspicious, noAsyncPromiseExecutor, noCatchAssign, noClassAssign, noCompareNegZero, noConstantBinaryExpressions, noControlCharactersInRegex, noDebugger (+26 more)

### Community 16 - "Dialogs & Color Picker"
Cohesion: 0.12
Nodes (22): apiKeyController(), buildController(), openEventStream(), sendEvent(), sleep(), environmentController(), globalVariablesController(), integrationController() (+14 more)

### Community 17 - "UI Inputs & Popover"
Cohesion: 0.06
Nodes (34): AcceptInvitationDTO, AddUserToProjectDTO, PopulatedProject, UpdateUserRoleDTO, UserProjectWithProject, AuthTokenDataDTO, BusinessException, ErrorDetails (+26 more)

### Community 18 - "Frontend Routes"
Cohesion: 0.05
Nodes (35): EXTERNAL_NAV_ITEMS, Sidebar, SidebarProps, AccountActivation, AddAzure, AddGithub, AddGitlab, AddMicrofrontend (+27 more)

### Community 19 - "Backend Controller Layer"
Cohesion: 0.12
Nodes (16): Microfrontend, DeploymentMicrofrontendCardProps, AddNewMicrofrontendCard(), AddNewMicrofrontendCardProps, IDangerZoneRemoveMicrofrontendProps, MicrofrontendCard(), MicrofrontendCardProps, MicrofrontendFlowNode() (+8 more)

### Community 21 - "Sidebar & Theme Toggle"
Cohesion: 0.22
Nodes (7): authorizationController(), GoogleTokenResponse, configurationController(), StartupUserRegistrationDTO, isRedirectToVersion(), AuthenticationMethod, ConfigResponseDTO

### Community 22 - "ApiKeyService.ts"
Cohesion: 0.25
Nodes (12): NpmPackageInfo, DependencyKind, comparePrerelease(), compareVersions(), diffVersions(), minVersionOfRange(), normalizeRange(), ParsedVersion (+4 more)

### Community 23 - "Frontend API Client"
Cohesion: 0.04
Nodes (45): apiKeys, api_key_created, api_key_created_description, api_keys, api_keys_description, confirm_delete, copy_and_close, copy_to_clipboard (+37 more)

### Community 24 - "Auth Wrappers & Context"
Cohesion: 0.06
Nodes (35): builds, empty, live, refresh, status, subtitle, table, title (+27 more)

### Community 25 - "Project Docs & Tooling"
Cohesion: 0.12
Nodes (17): mfe-orchestrator Docker Service, MongoDB Docker Service (production compose), Redis Docker Service (production compose), MongoDB Docker Service (local development, port 27018), Redis Docker Service (local development, port 6380), Anonymous telemetry 📡, Environment variables, Full precedence (+9 more)

### Community 26 - "Biome Correctness Rules"
Cohesion: 0.08
Nodes (26): noConstantCondition, noConstAssign, noEmptyCharacterClassInRegex, noEmptyPattern, noGlobalObjectCalls, noInvalidBuiltinInstantiation, noInvalidConstructorSuper, noNonoctalDecimalEscape (+18 more)

### Community 27 - "Tabs Component"
Cohesion: 0.11
Nodes (17): ICodeIntegrationDTO, ICodeIntegrationRequestDTO, IServeMicrofrontend, MicrofrontendCompiler, MicrofrontendFramework, MicrofrontendStackSource, useServeApi(), CodeIntegration() (+9 more)

### Community 28 - "API Key Domain"
Cohesion: 0.06
Nodes (35): builds, empty, live, refresh, status, subtitle, table, title (+27 more)

### Community 29 - "API Hooks & Pages"
Cohesion: 0.20
Nodes (12): ApiKey, ApiKeyRole, apiKeySchema, ApiKeyStatus, IApiKey, IApiKeyDocument, BuiltFrontend, builtFrontendSchema (+4 more)

### Community 30 - "Code Repositories API"
Cohesion: 0.09
Nodes (22): Additional Types, Best Practices, Branch Naming, Breaking Changes, Commit Conventions, Commit Message Format, Commit Message Template, Commit Types (+14 more)

### Community 31 - "Code Repository Service"
Cohesion: 0.12
Nodes (6): CodeRepository, ICodeRepository, CodeRepositoryService, retypedSecret(), CreateAzureDevOpsRepositoryDTO, CreateGitlabRepositoryDto

### Community 32 - "Login & Social Buttons"
Cohesion: 0.04
Nodes (66): GitHub Code Repository Provider, GitHub Logo (Octocat Mark), Button(), PageHead(), PageHeadProps, AddRepositoryAzureDTO, AddRepositoryGithubDTO, AddRepositoryGitlabDTO (+58 more)

### Community 34 - "Frontend App TSConfig"
Cohesion: 0.10
Nodes (20): compilerOptions, allowImportingTsExtensions, isolatedModules, jsx, lib, module, moduleDetection, moduleResolution (+12 more)

### Community 35 - "Dropdown Menu & Sidebar"
Cohesion: 0.17
Nodes (16): FederationFileChange, FederationIntegrationApplyResult, FederationIntegrationPlan, FederationIntegrationStatus, IntegrationScope, MicrofrontendIntegrationPlan, MicrofrontendIntegrationResult, useIntegrationApi() (+8 more)

### Community 36 - "Google & S3 Storage Clients"
Cohesion: 0.15
Nodes (12): AuthenticationError, getSecret(), AuthUserDTO, getDataFromAuth0(), getDataFromGoogle(), getDataFromLocal(), getDataFromMsal(), getEntraIdIssuer() (+4 more)

### Community 37 - "GitHub Client Methods"
Cohesion: 0.11
Nodes (26): EntityPickerLabels, EntityPickerList(), EntityPickerListProps, PickableEntity, SinglePageHeaderProps, SinglePageLayout(), BuildRun, BuildStatus (+18 more)

### Community 38 - "Frontend Dev Dependencies"
Cohesion: 0.11
Nodes (18): devDependencies, autoprefixer, eslint, @eslint/js, eslint-plugin-react-hooks, eslint-plugin-react-refresh, globals, postcss (+10 more)

### Community 39 - "Deployments & Variables API"
Cohesion: 0.24
Nodes (12): CanaryDeploymentType, CanaryType, DimensionsDTO, HostedOn, PositionDTO, RelationDTO, ReleaseSection(), ReleaseSectionProps (+4 more)

### Community 40 - "Azure DevOps Client Types"
Cohesion: 0.26
Nodes (10): ProjectUser, useProjectUserApi(), AddUserButton(), AddUserButtonProps, gravatarHash(), UserPicture(), getUserFullName(), getUserInitials() (+2 more)

### Community 41 - "E2E Package Config"
Cohesion: 0.11
Nodes (18): devDependencies, dotenv, mailinator-client, @playwright/test, @types/node, typescript, name, packageManager (+10 more)

### Community 42 - "AuthenticationMethod.ts"
Cohesion: 0.15
Nodes (24): NODE_ENVS, asList(), contextOf(), encryptedFields(), EncryptedFieldsOptions, isTraversable(), DEPLOYMENT, STORAGE (+16 more)

### Community 43 - "E2E TSConfig"
Cohesion: 0.12
Nodes (16): compilerOptions, allowSyntheticDefaultImports, esModuleInterop, forceConsistentCasingInFileNames, isolatedModules, lib, module, moduleResolution (+8 more)

### Community 44 - "FrontendIntegration.tsx"
Cohesion: 0.13
Nodes (24): CodeRepositoryProvider, CodeRepositoryCreateInput, CodeRepositoryUpdateInput, DEPENDENCY_SECTIONS, ManifestFile, ManifestSnapshot, PackageManifest, SECTION_BY_KIND (+16 more)

### Community 45 - "Canary Users Domain"
Cohesion: 0.17
Nodes (8): DeploymentToCanaryUsers, deploymentToCanaryUsersSchema, IDeploymentToCanaryUsers, DeploymentCanaryUsersService, A_DEPLOYMENT_ID, A_SESSION, anExistingRow(), tick()

### Community 46 - "Form Input Fields"
Cohesion: 0.09
Nodes (49): CreateOrganizationFormProps, OrganizationPickerListProps, InvitationKind, InvitationRow, PENDING_INVITATIONS_QUERY_KEY, PENDING_ORGANIZATION_INVITATIONS_QUERY_KEY, PendingInvitationsList(), PendingInvitationsListProps (+41 more)

### Community 47 - "Environment List & Drag-Drop"
Cohesion: 0.04
Nodes (45): apiKeys, api_key_created, api_key_created_description, api_keys, api_keys_description, confirm_delete, copy_and_close, copy_to_clipboard (+37 more)

### Community 48 - "Frontend Node TSConfig"
Cohesion: 0.12
Nodes (15): compilerOptions, allowImportingTsExtensions, isolatedModules, lib, module, moduleDetection, moduleResolution, noEmit (+7 more)

### Community 49 - "Telemetry Service & Config"
Cohesion: 0.11
Nodes (24): telemetryController(), build(), initSentry(), start(), logTelemetryNotice(), TelemetryService, AppInstance, fastify (+16 more)

### Community 50 - "User & Invitation API"
Cohesion: 0.10
Nodes (22): MicrofrontendType, federationName(), FederationRemote, COMMIT_MESSAGES, FederationFileChangeDTO, FederationIntegrationApplyResultDTO, FederationIntegrationPlanDTO, FederationIntegrationService (+14 more)

### Community 51 - "integration"
Cohesion: 0.05
Nodes (40): compiler_label, config_step, description, detected_stack, federation_tab, framework_label, install_step, integrate_all_button (+32 more)

### Community 52 - "Azure DevOps Client"
Cohesion: 0.05
Nodes (44): project_users, accepted, already_invited, already_member, cannot_invite_self, confirm_remove, confirm_remove_description, confirm_remove_title (+36 more)

### Community 53 - "Error Handler Middleware"
Cohesion: 0.23
Nodes (10): DeploymentDTO, useDeploymentsApi(), GlobalVariable, IServe, DeploymentList(), DeploymentListProps, Integration(), DeploymentGate() (+2 more)

### Community 54 - "Fastify App Bootstrap"
Cohesion: 0.11
Nodes (18): [1.2.0] - 2026-07-14, [2.0.0] - 2026-08-04, [2.2.0] - 2026-08-07, [2.3.0] - 2026-08-11, Added, Added, Automated Changelog Generation, Changed (+10 more)

### Community 55 - "UserModel.ts"
Cohesion: 0.10
Nodes (20): action, alreadyImported, clearSearch, description, failedCount_one, failedCount_other, import_one, import_other (+12 more)

### Community 56 - "useUserApi.ts"
Cohesion: 0.29
Nodes (7): hint, remove, subtitle, title, upload, uploading, avatar

### Community 57 - "Project Wizard Service"
Cohesion: 0.22
Nodes (11): CloneRepositoryPopover(), CloneRepositoryPopoverProps, CloneUrlRow(), CloneUrlRowProps, IdeCloneButtonProps, asNonEmptyString(), buildGitCloneCommand(), buildIntelliJCloneUrl() (+3 more)

### Community 58 - "Backend TSConfig"
Cohesion: 0.14
Nodes (13): compilerOptions, esModuleInterop, forceConsistentCasingInFileNames, isolatedModules, module, moduleResolution, outDir, resolveJsonModule (+5 more)

### Community 59 - "Code Repository Model"
Cohesion: 0.11
Nodes (21): CreateRepositoryRequest, GithubAccessTokenResponse, GithubAccessTokenRquest, GithubBaseDTO, GithubBranch, GithubContentsResponse, GithubCreateBranchDTO, GithubFileContent (+13 more)

### Community 60 - "CodeRepositoryController.ts"
Cohesion: 0.13
Nodes (15): description, integrate_button, integrate_dialog_description, integrate_dialog_title, integrate_nothing_to_do, integrate_partial, integrate_success, javascript_auto_note (+7 more)

### Community 61 - "MicrofrontendController.ts"
Cohesion: 0.05
Nodes (43): project_users, accepted, already_invited, already_member, cannot_invite_self, confirm_remove, confirm_remove_title, confirm_revoke (+35 more)

### Community 62 - "Telemetry Flags & Plugins"
Cohesion: 0.12
Nodes (15): Architecture Overview, Authentication & Authorization, Backend Controller Pattern, Backend Key Directories, Backend Service Pattern, Common Implementation Patterns, Critical Rules, Development Workflows (+7 more)

### Community 63 - "Frontend App Entry & Theme"
Cohesion: 0.12
Nodes (16): Anonymous telemetry 📡, Code Quality, Contributing 🤝, Development Guidelines, Development Workflow, Documentation 📚, Environment variables 🔧, How it works (+8 more)

### Community 64 - "Frontend Base TSConfig"
Cohesion: 0.17
Nodes (11): compilerOptions, allowJs, noImplicitAny, noUnusedLocals, noUnusedParameters, paths, skipLibCheck, strictNullChecks (+3 more)

### Community 65 - "MicrofrontendService.ts"
Cohesion: 0.16
Nodes (8): projectUserController(), createBusinessException(), IProject, IUserProject, UserProject, ProjectService, ProjectSummaryDTO, UserProjectService

### Community 66 - "IntegrationService.ts"
Cohesion: 0.38
Nodes (6): drainFrames(), EventStreamState, ParsedFrame, parseFrame(), useEventStream(), UseEventStreamOptions

### Community 67 - "errorHandler.ts"
Cohesion: 0.33
Nodes (5): marketController(), IMarket, Market, marketSchema, MarketService

### Community 68 - "Backend Dev Dependencies"
Cohesion: 0.22
Nodes (9): Basic Feature, Breaking Change, Bug Fix, Documentation, Examples, Feature with Scope, Multi-paragraph Commit, Performance Improvement (+1 more)

### Community 69 - "AuthenticationMethod.ts"
Cohesion: 0.23
Nodes (9): CodeRepositoryImportService, ImportableRepository, asId(), asNonEmptyString(), asNumber(), buildUniqueSlug(), NormalizedRepository, normalizeRepository() (+1 more)

### Community 70 - "Microfrontend Build Dialog"
Cohesion: 0.11
Nodes (17): CI/CD, Configuration, Debug tests, Documentation, E2E Tests with Playwright, Generate tests with Codegen, Installation, Prerequisites (+9 more)

### Community 71 - "integration"
Cohesion: 0.05
Nodes (40): compiler_label, config_step, description, detected_stack, federation_tab, framework_label, install_step, integrate_all_button (+32 more)

### Community 72 - "[Unreleased]"
Cohesion: 0.06
Nodes (35): description, success, title, auth, account_activation, already_have_account, confirm_password, create_account (+27 more)

### Community 74 - "Backend Test TSConfig"
Cohesion: 0.33
Nodes (5): compilerOptions, noEmit, exclude, extends, include

### Community 75 - "Biome Style Rules"
Cohesion: 0.11
Nodes (18): noAdjacentSpacesInRegex, noArguments, noExtraBooleanCast, noUselessCatch, noUselessEscapeInRegex, noUselessTypeConstraint, linter, enabled (+10 more)

### Community 76 - "AuthenticationMethod.ts"
Cohesion: 0.13
Nodes (15): description, integrate_button, integrate_dialog_description, integrate_dialog_title, integrate_nothing_to_do, integrate_partial, integrate_success, javascript_auto_note (+7 more)

### Community 77 - "Switch.rhf.tsx"
Cohesion: 0.06
Nodes (35): description, success, title, auth, account_activation, already_have_account, confirm_password, create_account (+27 more)

### Community 79 - "AzureDevOpsClient.ts"
Cohesion: 0.10
Nodes (19): AzureAccessTokenRequest, AzureAccessTokenResponse, AzureDevOpsBranch, AzureDevOpsBranchDTO, AzureDevOpsBuildsResponse, AzureDevOpsItem, AzureDevOpsPipeline, AzureDevOpsProject (+11 more)

### Community 80 - "BusinessException"
Cohesion: 0.10
Nodes (20): TextareaFieldProps, AzureAuthConfig, AzureStorageConfig, CreateStorageDTO, GoogleAuthConfig, GoogleStorageConfig, IStorageAuth, S3ClientConfig (+12 more)

### Community 81 - "FastifyInstance"
Cohesion: 0.07
Nodes (11): CustomError, EnvironmentHeaderNotFoundError, InvalidCredentialsError, UserAlreadyExistsError, UserCannotAccessThisDeploymentError, UserCannotAccessThisEnvironmentError, UserCannotAccessThisOrganizationError, UserCannotAccessThisProjectError (+3 more)

### Community 82 - "App.tsx"
Cohesion: 0.33
Nodes (6): integration, microfrontend_select_label, microfrontend_select_placeholder, sidebar_title, subtitle, title

### Community 83 - "UserService"
Cohesion: 0.33
Nodes (6): integration, microfrontend_select_label, microfrontend_select_placeholder, sidebar_title, subtitle, title

### Community 84 - "Backend Postbuild Script"
Cohesion: 0.40
Nodes (4): copyDirectory(), fs, path, postBuild()

### Community 85 - "Dependencies.tsx"
Cohesion: 0.09
Nodes (26): AlignmentApplyRequest, AlignmentApplyResult, AlignmentApplyResultItem, AlignmentPlan, CodeRepositoryProvider, Dependency, DependencyAlignmentIssue, DependencyKind (+18 more)

### Community 86 - "autorization.ts"
Cohesion: 0.06
Nodes (39): ProjectPickerListProps, CreateEnvironmentDTO, useEnvironmentsApi(), AddUserToProjectDTO, Project, ProjectSummaryDTO, RoleInProject, useProjectApi() (+31 more)

### Community 87 - "Sidebar.tsx"
Cohesion: 0.08
Nodes (44): App(), queryClient, CalendarField(), CalendarFieldProps, LanguageSelector(), LanguageSelectorProps, ThemeToggle(), ThemeToggleProps (+36 more)

### Community 88 - "IMarket"
Cohesion: 0.50
Nodes (4): [3.1.0] - 2026-08-16, Changed, Fixed, Tests

### Community 90 - "IntegrationService.ts"
Cohesion: 0.26
Nodes (4): MicrofrontendDependencyService, mapWithConcurrency(), toErrorMessage(), MicrofrontendAlignmentChangeDTO

### Community 91 - "Switch Component"
Cohesion: 0.09
Nodes (22): AuthConfig, AzureStorageConfig, AuthConfig, GoogleStorageConfig, S3ClientConfig, IStorage, IStorageAuth, Storage (+14 more)

### Community 93 - "MicrofrontendModel.ts"
Cohesion: 0.15
Nodes (13): errors, hint, notifications, remove, subtitle, title, upload, uploading (+5 more)

### Community 94 - "ApiStatusHandler Component"
Cohesion: 0.07
Nodes (28): common, actions, active, add, back, cancel, close, configuration (+20 more)

### Community 95 - "CI/CD Deploy Workflows"
Cohesion: 0.67
Nodes (3): Deploy to Server Workflow, Docker Build Workflow, Monorepo Build & Test Workflow

### Community 98 - "AWS Provider Branding"
Cohesion: 1.00
Nodes (3): Amazon Web Services (AWS), Cloud Storage Provider Selection UI, AWS Logo (SVG)

### Community 99 - "Registration E2E Test"
Cohesion: 0.06
Nodes (80): activateAccountFromEmail(), AppSession, canSendEmail(), CreatedOrganization, CreatedProject, createOrganizationViaApi(), createOrganizationViaUi(), createProjectViaApi() (+72 more)

### Community 100 - "app"
Cohesion: 0.22
Nodes (9): app, error, name, page_title, back_home, generic, not_found, not_found_description (+1 more)

### Community 102 - "Azure Provider Branding"
Cohesion: 0.67
Nodes (3): Cloud Provider Selection Option in Frontend UI, Microsoft Azure Logo (SVG), Microsoft Azure Cloud Platform

### Community 103 - "Google Cloud Branding"
Cohesion: 0.67
Nodes (3): Cloud Provider Option in Frontend UI, Google Cloud Platform, Google Cloud Logo (SVG)

### Community 110 - "Local Installation for development 🛠️"
Cohesion: 0.11
Nodes (23): AuthenticationType, createUrl(), doRequest(), getUTMFields(), IClientRequestData, IClientRequestMetadata, IUTMFields, out (+15 more)

### Community 117 - "Start Complete Script"
Cohesion: 0.20
Nodes (9): API Calls, Architecture, Development Commands, Frontend, Full Documentation, Git Workflow, Key Features, Project Rules (+1 more)

### Community 120 - "Multipart Plugin"
Cohesion: 0.50
Nodes (4): language, change, english, italian

### Community 128 - "canary_users"
Cohesion: 0.07
Nodes (28): author, description, devDependencies, tsx, @types/adm-zip, @types/fs-extra, @types/jsonwebtoken, @types/node (+20 more)

### Community 129 - "mfe-orchestrator Helm chart"
Cohesion: 0.22
Nodes (9): Environment variables, Image version, Ingress, Install, mfe-orchestrator Helm chart, MongoDB and Redis, Storage, Values reference (+1 more)

### Community 130 - "Security Policy"
Cohesion: 0.17
Nodes (7): Dependabot Weekly npm Updates (frontend and backend), Dependencies, Reporting a Vulnerability, Secure Development, Security Policy, Security Updates, Supported Versions

### Community 131 - "common"
Cohesion: 0.07
Nodes (28): common, actions, active, add, back, cancel, close, configuration (+20 more)

### Community 132 - "README.md"
Cohesion: 0.04
Nodes (48): redeploy, view_canary_users, add_button, add_description, add_title, added_success, columns, disable_selected (+40 more)

### Community 133 - "start-backend.sh"
Cohesion: 0.60
Nodes (4): log(), mongo_eval(), start-backend.sh script, wait_for()

### Community 134 - "deployments"
Cohesion: 0.04
Nodes (48): redeploy, view_canary_users, add_button, add_description, add_title, added_success, columns, disable_selected (+40 more)

### Community 135 - "projectWizardStateMachine.ts"
Cohesion: 0.33
Nodes (6): setup, description, project_name, project_name_min_length, project_name_placeholder, title

### Community 136 - "GitHub Actions Workflows"
Cohesion: 0.40
Nodes (4): Adding New Workflows, Available Workflows, E2E Tests (`e2e-tests.yml`), GitHub Actions Workflows

### Community 138 - "scripts"
Cohesion: 0.14
Nodes (15): ApiStatusHandler(), useDependenciesApi(), Market, useMarketApi(), Dependencies(), toDefaultBranches(), AlignPeerDependenciesDialog(), AlignPeerDependenciesDialogProps (+7 more)

### Community 139 - "FederationConfigService"
Cohesion: 0.06
Nodes (33): AcceptInvitationDTO, AddUserToOrganizationDTO, organizationUserController(), UpdateUserRoleDTO, IOrganization, Organization, organizationSchema, IUserOrganization (+25 more)

### Community 140 - "[2.3.0] - 2026-08-11"
Cohesion: 0.12
Nodes (21): AzureDevOpsBuild, GithubWorkflowRun, getByProjectId, snapshot, BuildStatusService, CachedSnapshot, normalizeAzureStatus(), normalizeGithubStatus() (+13 more)

### Community 141 - "Monorepo Architecture (pnpm workspace + Turbo)"
Cohesion: 0.67
Nodes (3): Monorepo Architecture (pnpm workspace + Turbo), Lefthook Pre-commit Hooks (Biome check + typecheck), pnpm Workspace Packages (frontend, backend, e2e)

### Community 142 - "package.json"
Cohesion: 0.25
Nodes (7): engines, node, name, packageManager, private, type, version

### Community 144 - "CLAUDE.md"
Cohesion: 0.50
Nodes (3): Costo di contesto, graphify, i18n

### Community 161 - "scripts"
Cohesion: 0.25
Nodes (8): scripts, build, build:dev, dev, format, lint, preview, typecheck

### Community 162 - "environmentVariables"
Cohesion: 0.17
Nodes (12): personalDataUpdated, email, emailNotEditable, name, subtitle, surname, title, profile (+4 more)

### Community 163 - "platform.json"
Cohesion: 0.12
Nodes (15): color_picker, select_color, language, change, english, italian, theme, dark (+7 more)

### Community 164 - "dashboard"
Cohesion: 0.11
Nodes (19): clearSearch, description, filteredCount_one, filteredCount_other, filterStatus, noResultsDescription, noResultsTitle, resetFilters (+11 more)

### Community 165 - "CreateApiKeyForm.tsx"
Cohesion: 0.11
Nodes (17): css, parser, files, ignoreUnknown, includes, selfCloseVoidElements, html, formatter (+9 more)

### Community 166 - "project"
Cohesion: 0.11
Nodes (19): project, create, create_new, create_new_project, description, description_placeholder, name, name_placeholder (+11 more)

### Community 167 - "dashboard"
Cohesion: 0.16
Nodes (9): COMPILER_MARKERS, CONFIG_CANDIDATES, FEDERATION_PLUGIN_BY_COMPILER, FRAMEWORK_MARKERS, MicrofrontendStackDetectionDTO, PackageManifest, StackDetectionService, parseCompiler() (+1 more)

### Community 168 - "theme"
Cohesion: 0.22
Nodes (9): delete, delete, subtitle, title, button, confirmation, description, title (+1 more)

### Community 169 - "form"
Cohesion: 0.11
Nodes (19): form, color, color_placeholder, color_required, description, description_placeholder, domains, domains_placeholder (+11 more)

### Community 170 - "environmentVariables"
Cohesion: 0.11
Nodes (19): environmentVariables, actions, addVariable, created_success, deleteConfirmation, deleted_success, deleteVariable, editVariable (+11 more)

### Community 171 - "project"
Cohesion: 0.11
Nodes (19): project, create, create_new, create_new_project, description, description_placeholder, name, name_placeholder (+11 more)

### Community 172 - "platform.json"
Cohesion: 0.13
Nodes (14): app, name, page_title, setup, description, project_name, project_name_min_length, project_name_placeholder (+6 more)

### Community 173 - "Local Installation for development 🛠️"
Cohesion: 0.40
Nodes (5): Available Commands, Development URLs, Local Installation for development 🛠️, Prerequisites, Quick Start

### Community 174 - "EmailSenderService"
Cohesion: 0.07
Nodes (34): EnvironmentNotFoundError, configSchema, Configuration, IConfiguration, Deployment, deploymentSchema, Environment, environmentSchema (+26 more)

### Community 175 - "environment"
Cohesion: 0.13
Nodes (15): environment, add_environment, color, color_tooltip, configure, create_success, created_success_message, custom_environments (+7 more)

### Community 176 - "settings"
Cohesion: 0.05
Nodes (41): delete, subtitle, title, button, confirmation, description, dialog, error (+33 more)

### Community 177 - "theme"
Cohesion: 0.22
Nodes (4): HOST_TYPE_LABEL_KEYS, HOST_TYPE_ICONS, MicrofrontendsTable(), MicrofrontendsTableProps

### Community 178 - "build"
Cohesion: 0.17
Nodes (12): branch, building, buildSuccess, custom_version, defaultBranch, loadingBranches, selectBranch, startBuild (+4 more)

### Community 180 - "delete"
Cohesion: 0.15
Nodes (13): projectDeleted, projectNameUpdated, projectNameUpdateFailed, id, name, slug, title, settings (+5 more)

### Community 182 - "errorHandler.ts"
Cohesion: 0.19
Nodes (4): StartupController(), UserController(), UserAvatar, UserService

### Community 184 - "project_invitation"
Cohesion: 0.13
Nodes (15): project_invitation, accept, accept_short, accepted, decline, declined, description, error_description (+7 more)

### Community 185 - "project_invitation"
Cohesion: 0.13
Nodes (15): project_invitation, accept, accept_short, accepted, decline, declined, description, error_description (+7 more)

### Community 186 - "AzureStorageClient"
Cohesion: 0.33
Nodes (6): error, back_home, generic, not_found, not_found_description, not_found_title

### Community 188 - "NpmRegistryClient.ts"
Cohesion: 0.25
Nodes (8): dialog, confirmation, confirmationText, confirmButton, deleting, description, title, warning

### Community 189 - "stats"
Cohesion: 0.25
Nodes (8): stats, apiKeys, codeRepositories, environments, storages, teamMembers, title, viewAll

### Community 190 - "marketing"
Cohesion: 0.29
Nodes (7): grantedAt, notifications, subtitle, title, granted, withdrawn, marketing

### Community 191 - "card"
Cohesion: 0.29
Nodes (7): build, canary, repository, storage, users, version, card

### Community 193 - "DeploymentService"
Cohesion: 0.11
Nodes (8): IDeployment, IGlobalVariable, DeploymentService, EnvironmentService, GlobalVariablesService, EnvironmentDTO, GlobalVariableDTO, toObjectId()

### Community 194 - "GitlabClient.ts"
Cohesion: 0.18
Nodes (9): AddGroupSecretRequest, CheckGroupSecretExistsRequest, CommitAction, CommitFilesRequest, CreateRepositoryRequest, GitLabBranch, GitLabGroup, GitLabPipeline (+1 more)

### Community 195 - "table"
Cohesion: 0.29
Nodes (7): table, actions, description, is_production, name, no_environments, slug

### Community 196 - "form"
Cohesion: 0.33
Nodes (6): cancel, create, create_title, edit_title, update, form

### Community 198 - "curl"
Cohesion: 0.50
Nodes (4): description, step2, title, curl

### Community 199 - "NpmRegistryClient.ts"
Cohesion: 0.24
Nodes (4): cache, CacheEntry, NpmAbbreviatedPackument, NpmRegistryClient

### Community 203 - "validation"
Cohesion: 0.50
Nodes (4): validation, name_required, slug_invalid, slug_required

### Community 206 - "personalData"
Cohesion: 0.20
Nodes (10): email, emailNotEditable, name, subtitle, surname, title, profile, personalData (+2 more)

### Community 208 - "[1.0.0] - 2025-12-07"
Cohesion: 0.33
Nodes (6): [1.0.0] - 2025-12-07, Added, Changed, Documentation, Fixed, Removed

### Community 209 - "curl"
Cohesion: 0.50
Nodes (4): description, step2, title, curl

### Community 210 - "assist"
Cohesion: 0.40
Nodes (5): source, assist, actions, enabled, organizeImports

### Community 211 - "[2.1.0] - 2026-08-06"
Cohesion: 0.40
Nodes (5): [2.1.0] - 2026-08-06, Added, Changed, Documentation, Fixed

### Community 213 - "[3.0.0] - 2026-08-16"
Cohesion: 0.40
Nodes (5): [3.0.0] - 2026-08-16, Added, Changed, CI, Fixed

### Community 214 - "[Unreleased]"
Cohesion: 0.40
Nodes (5): Added, Changed, Documentation, Fixed, [Unreleased]

### Community 215 - "Scopes (Optional)"
Cohesion: 0.40
Nodes (5): Backend Layer Scopes, Frontend Layer Scopes, General Scopes, Module Scopes, Scopes (Optional)

### Community 217 - "DeploymentController.ts"
Cohesion: 0.29
Nodes (4): deploymentController(), deploymentController(), DeploymentCanaryUsersDTO, DeploymentDTO

## Knowledge Gaps
- **1800 isolated node(s):** `description`, `main`, `start`, `st`, `dev` (+1795 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **46 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `useUserApi()` connect `Sidebar.tsx` to `Backend Controllers & Auth`, `GitHub Client Methods`, `Code Repositories UI`, `Local Installation for development 🛠️`, `errorHandler.ts`?**
  _High betweenness centrality (0.171) - this node is a cross-community bridge._
- **Why does `toObjectId()` connect `DeploymentService` to `Deployment Service & Models`, `UI Cards & Alerts`, `Biome Configuration`, `Azure Storage & Microfrontend Model`, `FederationConfigService`, `[2.3.0] - 2026-08-11`, `UI Inputs & Popover`, `Tables & Layout Components`, `API Hooks & Pages`, `Code Repository Service`, `Canary Users Domain`, `EmailSenderService`, `User & Invitation API`, `errorHandler.ts`, `BaseAuthorizedService`, `MicrofrontendService.ts`, `AuthenticationMethod.ts`, `.getMicrofrontendVersion`, `.create`, `Switch Component`?**
  _High betweenness centrality (0.158) - this node is a cross-community bridge._
- **Why does `UserButton()` connect `Sidebar.tsx` to `Backend Controllers & Auth`, `Frontend Routes`, `React Misc Hooks`?**
  _High betweenness centrality (0.028) - this node is a cross-community bridge._
- **Are the 2 inferred relationships involving `toObjectId()` (e.g. with `.getRepositoriesById()` and `.bulkDelete()`) actually correct?**
  _`toObjectId()` has 2 INFERRED edges - model-reasoned connections that need verification._
- **What connects `description`, `main`, `start` to the rest of the system?**
  _1803 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Deployment Service & Models` be split into smaller, more focused modules?**
  _Cohesion score 0.11153846153846154 - nodes in this community are weakly interconnected._
- **Should `Frontend Dependencies` be split into smaller, more focused modules?**
  _Cohesion score 0.0625 - nodes in this community are weakly interconnected._