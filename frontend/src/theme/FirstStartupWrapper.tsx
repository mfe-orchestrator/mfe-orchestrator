...
const RegisterFirstUser: React.FC = () => {
    ...
    return (
        <AuthenticationLayout title={t("setup.title")} description={t("setup.description")}>
            <FormProvider {...form}>
...
const FirstStartupWrapper: React.FC<React.PropsWithChildren> = ({ children }) => {
    ...
    return <ApiStatusHandler queries={[usersQuery]}>{usersQuery.data ? <>{children}</> : <RegisterFirstUser />}</ApiStatusHandler>
}
