...
const Sidebar = React.forwardRef<HTMLDivElement, SidebarProps>(
  ({ className, sidebarHeader, mainNavItems, isCollapsed, toggleCollapsed, ...props }, ref) => {
    ...
    const sidebarStyle = `md:h-sidebar md:py-6 group ${
      !isCollapsed ? "md:w-64 md:px-3" : "md:w-20 md:px-2"
    }`;

    return (
      <div
        id="sidebar_container"
        className={cn(navBarStyle, sidebarStyle, isMenuVisible && "h-sidebar", className)}>
        <div
          className={`flex items-center justify-between md:mb-12 ${
            isCollapsed ? "md:justify-center" : "md:justify-start"
          }`}>
          <div className="flex items-center md:p-2 gap-3">
            <div className="h-8 w-8 rounded-sm bg-orchestrator-accent flex items-center justify-center text-white font-bold">
              {!isCollapsed ? "MF" : "M"}
            </div>
            {!isCollapsed && (
              <span className="text-lg font-semibold text-orchestrator-secondary">
                {t("app.name")}
              </span>
            )}
          </div>
        ...
        <div
          ...
          } flex-col flex-grow border-t border-divider overflow-auto md:flex md:border-0 pt-2 md:pt-0 mt-4 md:mt-0`}>
          {mainNavItems && (
          ...
          <div className="flex flex-col gap-1 border-t border-divider py-2">
            <LanguageSelector
              isSidebarCollapsed={isCollapsed}
              dropdownContentSide={isMobile ? "bottom" : undefined}
              dropdownContentAlign={isMobile ? "start" : undefined}
            />
            <ThemeToggle
              isSidebarCollapsed={isCollapsed}
              dropdownContentSide={isMobile ? "bottom" : undefined}
            ...
            <NavItem
              ...
              icon={<BookOpen />}
              name="Documentation"
              href="https://mfe-orchestrator.dev/documentation/"
              isSidebarCollapsed={isCollapsed}
            />
            <NavItem
              type="secondary"
