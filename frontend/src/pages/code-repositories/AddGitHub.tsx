...
const AddGitHubRepositoryPage: React.FC = () => {
    ...
    const { getGithubOrganizations, getGithubUser, updateRepositoryGithub, getRepositoryById, revokeGithubGrant } = useCodeRepositoriesApi()
    ...
    const onUpdateGithubPage = async () => {
        // Revoke the OAuth grant so GitHub always shows the authorization page
        // (including the org "Grant" buttons) instead of silently redirecting back.
        try {
            await revokeGithubGrant(repositoryId, { silent: true })
        } catch {
    ...
    return (
        <SinglePageLayout title={t("codeRepositories.github.title")}>
            <ApiStatusHandler queries={[repositoryQuery, githubOrganizationQuery, githubUserQuery]} interceptError={false}>
