...
const AddGitHubRepositoryPage: React.FC = () => {
  ...
  const { getGithubOrganizations, getGithubUser, updateRepositoryGithub, getRepositoryById } =
    useCodeRepositoriesApi();
  ...
  const onSubmit = async (data: GitHubConnectionForm) => {
    ...
    await updateRepositoryGithub(repositoryId, {
      ...
      type:
        data.selectedOwner !== CodeRepositoryType.PERSONAL
          ? CodeRepositoryType.ORGANIZATION
          : CodeRepositoryType.PERSONAL,
      organizationId:
        data.selectedOwner !== CodeRepositoryType.PERSONAL ? data.selectedOwner : undefined,
      userName:
        data.selectedOwner === CodeRepositoryType.PERSONAL ? githubUserQuery.data.login : undefined,
    });
  ...
  const isError =
    repositoryQuery.isError || githubOrganizationQuery.isError || githubUserQuery.isError;

  return (
    <SinglePageLayout title={t("codeRepositories.github.title")}>
      <ApiStatusHandler
        queries={[repositoryQuery, githubOrganizationQuery, githubUserQuery]}
                  ...
                  <div className="flex items-center space-x-4 justify-between">
                    <div>
                      <h3 className="text-sm font-medium">
                        {t("codeRepositories.github.githubAccessTitle")}
                      </h3>
                      <p className="text-xs text-muted-foreground">
                        {t("codeRepositories.github.githubAccessDescription")}
                      </p>
                    </div>
                    <Button
                      type="button"
                  ...
                  <form
                    ...
                    <div className="space-y-3">
                      <label className="text-sm font-medium">
                        {t("codeRepositories.github.connection")}
                      </label>
                      {!organizations || organizations.length === 0 ? (
                        <div className="space-y-2">
                          <div className="flex items-center space-x-2 p-3 border rounded-md bg-muted">
                            ...
                            <span className="text-sm">
                              {t("codeRepositories.github.personalProfileAutoSelected")}
                            </span>
                          </div>
                          <p className="text-xs text-muted-foreground">
                        ...
                        <div className="space-y-3">
                          ...
                          />
                          <p className="text-xs text-muted-foreground">
                            {t("codeRepositories.github.selectFromOptions", {
