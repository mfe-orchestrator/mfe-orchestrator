...
const SingleEnviromentSee: React.FC<SingleEnviromentSeeProps> = ({ environment, onDelete, onEdit, disableIcons, dragHandleProps }) => {
            ...
            <div className="flex items-center gap-4">
                {dragHandleProps && (
                    <span
                        ...
                        role="button"
                        tabIndex={0}
                        aria-label={t("environment.page.reorder.handle", { name: environment.name })}
                        title={t("environment.page.reorder.handle", { name: environment.name })}
...
export const EnvironmentList: React.FC<EnvironmentListProps> = ({ environments, onSaveEnvironments }) => {
    ...
    const onSaveEnvironmentWrapper = async () => {
        setLoading(true)
        // The position inside the list is the environment order
        await onSaveEnvironments(environmentList.map((env, index) => ({ ...env, order: index })))
        setLoading(false)
    }

    const { draggingId, dragOverId, getHandleProps, getItemProps } = useDragAndDropOrder({
        items: environmentList || [],
        getId: (env, index) => env._id || String(index),
        onReorder: setEnvironmentList,
