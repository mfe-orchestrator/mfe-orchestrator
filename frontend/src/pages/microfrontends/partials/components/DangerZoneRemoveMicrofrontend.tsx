...
export const DangerZoneRemoveMicrofrontend: React.FC<IDangerZoneRemoveMicrofrontendProps> = ({
  microfrontend,
}) => {
    ...
    <Card className="border-destructive">
      <CardHeader>
        <h2 className="text-xl font-semibold text-destructive">
          {t("microfrontend.dangerZone.title")}
        </h2>
        <CardDescription className="text-destructive-active">
          {t("microfrontend.dangerZone.subtitle")}
        </CardDescription>
      </CardHeader>
        ...
        <div className="flex items-center justify-between gap-y-2 gap-x-4 flex-wrap">
          <div>
            <h3 className="text-lg font-medium m-0">
              {t("microfrontend.dangerZone.delete.title")}
            </h3>
            <p className="text-foreground-secondary m-0">
              {t("microfrontend.dangerZone.delete.description")}
            </p>
          </div>
          <Button
            variant="destructive"
        ...
        <DialogContent className="max-w-lg w-[calc(100vw-2rem)]">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                confirmationText !== microfrontend.name || deleteMicrofrontendMutation.isPending
              }
              className={cn(
                deleteMicrofrontendMutation.isPending && "opacity-50 cursor-not-allowed",
              )}>
              {deleteMicrofrontendMutation.isPending
                ? t("microfrontend.dangerZone.delete.dialog.deleting")
                : t("microfrontend.dangerZone.delete.dialog.confirmButton")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
