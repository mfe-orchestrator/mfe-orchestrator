  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {isEditing ? t('environmentVariables.editVariable') : t('environmentVariables.addVariable')}
          </DialogTitle>
        </DialogHeader>
        <FormProvider {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              <TextField 
                name="key" 
                label={t('environmentVariables.key')} 
                required
              />

            {values && values.map((envValue, index) => (
                <TextField 
                  name={"values."+index+".value"}
                  label={environments.find((e) => e._id === envValue.environmentId)?.name || envValue.environmentId}
              />
            ))}
