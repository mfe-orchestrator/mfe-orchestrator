    ...
    const renderStorageConfig = () => {
        switch (storageType) {
            ...
            case StorageType.AZURE: {
                ...
                return (
                    <div className="space-y-4">
                        <SelectField name="authConfig.authType" label={t("storage.fields.authType")} options={azureAuthTypes} required rules={{ required: t("validation.required") }} />

                        <TextField name="authConfig.containerName" label={t("storage.fields.containerName")} placeholder="my-container" required rules={{ required: t("validation.required") }} />
                        ...
                        {azureAuthType === "connectionString" && (
                            <div className="space-y-4 p-4 bg-muted border rounded-lg">
                                ...
                                <TextField name="authConfig.connectionString" label={t("storage.fields.connectionString")} type="password" required rules={{ required: true }} />
                            </div>
                        )}

                        {(azureAuthType === "sharedKey" || azureAuthType === "aad") && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <TextField name="authConfig.accountName" label={t("storage.fields.accountName")} placeholder="mystorageaccount" required rules={{ required: true }} />
                            </div>
                        )}
                        ...
                        {azureAuthType === "sharedKey" && (
                            <div className="space-y-4 p-4 bg-muted border rounded-lg">
                                ...
                                <TextField name="authConfig.accountKey" label={t("storage.fields.accountKey")} required type="password" rules={{ required: true }} />
                            </div>
                        )}
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>{t("storage.basicInformationDescription")}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        ...
                        {isEditMode && (
                            <Alert>
                                ...
                                <AlertDescription>{t("storage.typeCannotBeChanged")}</AlertDescription>
                            </Alert>
                        )}
                    </CardContent>
                </Card>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>{t("storage.providerConfigurationDescription", { storageType: selectedStorageType })}</CardDescription>
                    </CardHeader>
                    <CardContent>
...
const NewOrEditStoragePage = () => {
    ...
    return (
        <SinglePageLayout title={isEditMode ? t("storage.editStorage") : t("storage.newStorage")} description={isEditMode ? t("storage.editStorageDescription") : t("storage.newStorageDescription")}>
            {isEditMode ? (
                <ApiDataFetcher queries={[storageQuery]}>
                    <StorageForm initialData={storageQuery.data} onSubmitSuccess={handleSubmitSuccess} onCancel={handleCancel} id={id} />
                </ApiDataFetcher>
            ) : (
                <StorageForm onSubmitSuccess={handleSubmitSuccess} onCancel={handleCancel} />
            )}
        </SinglePageLayout>
    )
}
