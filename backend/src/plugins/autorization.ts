...
const getDataFromAuth0 = async (
  ...
  const response = await axios.get(
    `https://${fastify.config.AUTH0_DOMAIN}/userinfo`,
    {
      headers: {
        Authorization: `Bearer ${authToken}`,
      },
  ...
  switch (issuer) {
    ...
    default:
      ...
      if (payload.iss == ISSUER) {
        ...
        payload.iss ==
        `https://login.microsoftonline.com/${fastify.config.AZURE_ENTRAID_TENANT_ID}/v2.0`
      ) {
...
export default fastifyPlugin(
  async (fastify: FastifyInstance) => {
    fastify.addHook("preHandler", async (request, response) => {
      ...
      const authMethod =
        request.routeOptions.config.authMethod || AuthenticationMethod.JWT;
      if (
      ...
      const authToken = request?.headers?.["authorization"]?.replace(
        "Bearer ",
        ""
      );
      ...
      if (user?.activateEmailToken) {
        if (
          user?.activateEmailExpires &&
          user.activateEmailExpires < new Date()
        ) {
          throw new AuthenticationError(
            "User not verified and the invitation is expired, please reset your password"
          );
        }
        throw new AuthenticationError(
      ...
      if (!user) {
        if (isFederatedAuth) {
          ...
          throw new AuthenticationError(
            "User found in authentication provider but not in database with email " +
              userData.email
          );
        }
      }
