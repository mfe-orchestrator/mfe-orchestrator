...
import CodeRepository, {
  ...
  CodeRepositoryType,
} from "../models/CodeRepositoryModel";
import {
  BusinessException,
  createBusinessException,
} from "../errors/BusinessException";
...
export class CodeRepositoryService extends BaseAuthorizedService {
  ...
  async getBranches(
    ...
    if (repository.provider === CodeRepositoryProvider.AZURE_DEV_OPS) {
      ...
      return new AzureDevOpsClient()
        .getBranches(
          repository.accessToken,
          repository.azureData.organization,
          repository.azureData.projectId,
          repositoryId
        )
  ...
  async injectSecretsToDeployOnGithub(repository: ICodeRepository) {
    ...
    const apiKey = await new ApiKeyService(this.user).create(
  ...
  async create(
    projectId: string,
    repositoryData: CodeRepositoryCreateInput
  ): Promise<ICodeRepository> {
    await this.ensureAccessToProject(projectId);
  ...
  async update(
    repositoryId: string,
    repositoryData: CodeRepositoryUpdateInput
  ): Promise<ICodeRepository | null> {
  ...
  async addRepositoryGithub(
    ...
    if (codeRepositoryId) {
      ...
      const userGithub = await githubClient.getUser(
  ...
  async editRepositoryAzureDevOps(
    body: CreateAzureDevOpsRepositoryDTO,
    repositoryId: string
  ) {
    const repository = await this.findById(repositoryId);
  ...
  async getAzureRepositories(
    ...
    return (
      await new AzureDevOpsClient().getRepositories(
        repository.accessToken,
        repository?.name,
        projectId
      )
  ...
  async getGitlabGroups(repositoryId: string): Promise<unknown> {
    ...
    if (
      repository.provider !== CodeRepositoryProvider.GITLAB ||
      !repository.gitlabData?.url
    ) {
      throw new BusinessException({
        code: "INVALID_PROVIDER",
        message: "Repository provider is not GitLab",
  ...
  async getGitlabPaths(
    ...
    return new GitLabClient(
      repository.gitlabData?.url,
      repository.accessToken
    ).getRepositoryPathsByGroupId(repository.gitlabData?.project);
  }
  ...
  async getGitlabRepositories(
    ...
    return new GitLabClient(
      repository.gitlabData?.url,
      repository.accessToken
    ).getRepositoriesByGroupId(groupId);
  }
  ...
  async getRepositories(repositoryId: string) {
    ...
    if (repository.provider === CodeRepositoryProvider.GITHUB) {
      ...
      return new GithubClient().getRepositories(
        repository.accessToken,
        repository.githubData?.type === CodeRepositoryType.ORGANIZATION
          ? repository.githubData.organizationId
          : undefined
      );
    }
    if (repository.provider === CodeRepositoryProvider.GITLAB) {
      ...
      return new GitLabClient(
        repository.gitlabData?.url,
        repository.accessToken
      ).getRepositoriesByGroupId(repository.gitlabData?.project);
    }
    if (repository.provider === CodeRepositoryProvider.AZURE_DEV_OPS) {
      if (
        !repository.azureData?.organization ||
