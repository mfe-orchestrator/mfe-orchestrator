...
export class StorageService extends BaseAuthorizedService {
  ...
  async getById(storageId: string | ObjectId): Promise<IStorage> {
    return runInTransaction(async (session) =>
      this.getByIdRaw(storageId, session)
    );
  }

  async getByIdRaw(
    storageId: string | ObjectId,
    session?: ClientSession
  ): Promise<IStorage> {
    const storagetIdObj =
      typeof storageId === "string" ? new Types.ObjectId(storageId) : storageId;
  ...
  async create(projectId: string, storageData: StorageDTO): Promise<IStorage> {
    return runInTransaction(async (session) =>
      this.createRaw(projectId, storageData, session)
    );
  }
  ...
  async updateRaw(
    ...
    const updated = await Storage.findByIdAndUpdate(
      storage._id,
      { ...storageData, projectId: storage.projectId },
  ...
  async update(storageId: string, storageData: StorageDTO): Promise<IStorage> {
    return runInTransaction(async (session) =>
      this.updateRaw(storageId, storageData, session)
    );
  }
