...
import { createStorageSchema, listStoragesSchema, storageIdSchema, updateStorageSchema } from "../schemas/storage.schema"
...
export default async function storageController(fastify: FastifyInstance) {
    ...
    fastify.get<{ Params: { projectId: string } }>("/projects/:projectId/storages", { schema: listStoragesSchema }, async (request, reply) => {
    ...
    fastify.get<{ Params: { storageId: string } }>("/storages/:storageId", { schema: storageIdSchema }, async (request, reply) => {
        const storage = await new StorageService(request.databaseUser).getById(request.params.storageId)
        return reply.send(storage)
    })

    // Add storage to project
    fastify.post<{ Body: StorageDTO }>("/storages", { schema: createStorageSchema }, async (request, reply) => {
    ...
    // Update storage
    fastify.put<{ Body: StorageDTO; Params: { storageId: string } }>("/storages/:storageId", { schema: updateStorageSchema }, async (request, reply) => {
        const storage = await new StorageService(request.databaseUser).update(request.params.storageId, request.body)
        return reply.send(storage)
    })

    // Delete storage
    fastify.delete<{ Params: { storageId: string } }>("/storages/:storageId", { schema: storageIdSchema }, async (request, reply) => {
        await new StorageService(request.databaseUser).delete(request.params.storageId)
        return reply.status(204).send()
    })

    // Set storage as default
    fastify.put<{ Params: { storageId: string } }>("/storages/:storageId/default", { schema: storageIdSchema }, async (request, reply) => {
