...
import { apiKeyIdSchema, createApiKeySchema, listApiKeysSchema } from "../schemas/apiKey.schema"
...
export default async function apiKeyController(fastify: FastifyInstance) {
    fastify.get<{ Params: { projectId: string } }>("/projects/:projectId/api-keys", { schema: listApiKeysSchema }, async (request, reply) => {
    ...
    fastify.post<{ Body: ApiKeyDTO }>("/api-keys", { schema: createApiKeySchema }, async (request, reply) => {
    ...
    fastify.put<{ Params: { apiKeyId: string } }>("/api-keys/:apiKeyId/revoke", { schema: apiKeyIdSchema }, async (request, reply) => {
        const apiKey = await new ApiKeyService(request.databaseUser).setStatus(request.params.apiKeyId, ApiKeyStatus.INACTIVE)
        return reply.send(apiKey)
    })

    fastify.delete<{ Params: { apiKeyId: string } }>("/api-keys/:apiKeyId", { schema: apiKeyIdSchema }, async (request, reply) => {
